				 ...
				 int channel, int msg, int result)
{
	...
	if (dw == 8) {
		brw_set_compression_control(p, BRW_COMPRESSION_NONE);
		mlen = 3;
		rlen = 4; /* SIMD8 sample return is not masked */
		len = 1;
	} else {
		brw_set_compression_control(p, BRW_COMPRESSION_COMPRESSED);
		mlen = 5;
	...
	brw_SAMPLE(p, sample_result(dw, G(result, len)), msg, src0,
		   channel+1, channel, WRITEMASK_X, 0,
		   rlen, mlen, true, simd(dw));

	brw_SAMPLE(p, sample_result(dw, R(result, len)), msg, src0,
		   channel+2, channel, WRITEMASK_X, 0,
		   rlen, mlen, true, simd(dw));

	brw_SAMPLE(p, sample_result(dw, B(result, len)), msg, src0,
			       ...
			       int channel, int msg, int result)
{
	...
	brw_MOV(p, brw_vec8_grf(R(result, len), 0), brw_vec8_grf(A(result, len), 0));
